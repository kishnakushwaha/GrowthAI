...
const WebsiteIntelligence = () => {
  ...
  const fetchIntelligence = async () => {
    ...
    try {
      ...
      // Reuse the signals endpoint but filter natively in client
  ...
  return (
    <div className="admin-page">
                  ...
                  <tr key={lead.id}>
                      ...
                      {(() => {
                        ...
                        return (
                          <a href={href} 
                             ...
                             style={{ fontSize: '0.75rem', display: 'inline-flex', alignItems: 'center', gap: '4px', marginTop: '6px', color: 'var(--primary)', textDecoration: 'none' }}>
                    ...
                    <td>
                      <span className={`hot-badge`} style={{ 
                        background: lead.cms_stack === 'WordPress' ? 'rgba(37, 99, 235, 0.2)' : 
                                   lead.cms_stack === 'Shopify' ? 'rgba(34, 197, 94, 0.2)' : 
                                   lead.cms_stack === 'Wix' ? 'rgba(156, 163, 175, 0.2)' : 'rgba(255,255,255,0.05)',
                        color: lead.cms_stack === 'WordPress' ? '#60a5fa' : 
                               lead.cms_stack === 'Shopify' ? '#4ade80' : 
                               lead.cms_stack === 'Wix' ? '#d1d5db' : '#9ca3af',
                      }}>
                        <Code size={12} /> {lead.cms_stack || 'Custom / Unknown'}
                      </span>
                    </td>
                    ...
                    <td>
                      {lead.pagespeed_mobile ? (
                        <span className="hot-badge" style={{
                          ...
                          <Zap size={12}/> {lead.pagespeed_mobile}/100 Mobile
                        </span>
                      ) : (
                    ...
                    <td style={{ width: '25%', whiteSpace: 'normal', fontSize: '0.85rem', color: 'var(--text-muted)', lineHeight: '1.4' }}>
                      {lead.pitch_angle || '—'}
                    </td>
                    
                    <td style={{ width: '40%', whiteSpace: 'normal', lineHeight: '1.4' }}>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        {lead.ai_human_summary ? (
                          <div style={{ background: 'rgba(0,0,0,0.2)', padding: '10px', borderRadius: '8px', borderLeft: '2px solid var(--primary)', fontSize: '0.8rem', color: 'rgba(255,255,255,0.9)' }}>
                            {lead.ai_human_summary}
                          </div>
                        ) : (
                          <div style={{ padding: '8px', borderRadius: '8px', borderLeft: '2px dashed rgba(255,255,255,0.1)', fontSize: '0.75rem', color: 'rgba(255,255,255,0.3)', fontStyle: 'italic' }}>
                            [NLP Engine Upgrade Applied] Fresh Scrape Required to generate Human Summary.
                          </div>
                        )}
                        ...
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-start', background: 'rgba(255,255,255,0.03)', padding: '8px', borderRadius: '6px' }}>
                          ...
                          <div style={{ display: 'flex', gap: '4px' }}>
                            ...
                            <a href={`#whatsapp?phone=${encodeURIComponent(lead.businesses?.phone || '')}&business=${encodeURIComponent(lead.businesses?.place_name || '')}&pitch=${encodeURIComponent(lead.ai_first_line || '')}&lead_id=${lead.id}`} 
                                className="btn-icon" style={{ background: '#22c55e', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center' }} title="Send via WhatsApp">
      ...
      {/* ENROLLMENT MODAL */}
      {enrollingLead && (
        <div className="modal-overlay" style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, backdropFilter: 'blur(5px)' }}>
          <div className="admin-card glass-panel" style={{ width: '450px', padding: '2rem', position: 'relative' }}>
            <button 
              onClick={() => setEnrollingLead(null)} 
              style={{ position: 'absolute', top: '1rem', right: '1rem', background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer' }}
            >
              <X size={24} />
            </button>
            <h2 style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '1.5rem' }}>
              <Play size={24} color="var(--primary)" /> Enroll in <span className="text-gradient">Sequence</span>
            </h2>
            <p className="text-muted" style={{ marginBottom: '1.5rem', fontSize: '0.9rem' }}>
              Enrolling <strong>{enrollingLead.businesses?.place_name}</strong> into a multi-step automated outreach campaign.
            </p>
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              >
                ...
                {campaigns.length > 0 ? campaigns.map(c => (
                  <option key={c.id} value={c.id}>{c.name} ({c.industry || 'All Industries'})</option>
                )) : (
                  <option disabled>No active campaigns found. Create one in Admin.</option>
                )}
              </select>
            </div>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              >
                {enrolling ? 'Enrolling...' : 'Start Automation'}
              </button>
              <button 
            ...
            <p style={{ marginTop: '1.5rem', fontSize: '0.75rem', color: 'var(--text-muted)', textAlign: 'center' }}>
              💡 Messages will be queued for the Chrome Extension to pick up.
            </p>
          </div>
        </div>
      )}
